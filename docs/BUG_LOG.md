...
|------|-----|------------|-----|-----------|--------|
| 2026-05-24 | En Route Verification missing from Stage Pipeline tab | Commit `ada6e80` removed `en_route_verification` from dashboard `STAGE_CONFIG` and `STAGE_ORDER`, so `/stages` skipped the card between En Route and Inventory Verification. | `pending` - restored dashboard stage config/order entry; `npm run build` passes. | Codex | Verified local build |
