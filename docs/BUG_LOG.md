...
|------|-----|------------|-----|-----------|--------|
| 2026-05-24 | Dashboard OTP modal returned nginx 502 Bad Gateway while creating orders | VPS API container was missing/stuck after stale Docker Compose container conflict, so nginx could not proxy `/api/*` to port 8080. | [`pending`](https://github.com/jpgyap-sudo/workflowautomation/commit/pending) — restored VPS Docker stack with `docker compose up -d`; verified public `/api/health` and `/api/auth/send-otp` returned 200. | Codex | ✅ Verified |
