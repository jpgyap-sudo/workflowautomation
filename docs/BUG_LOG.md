...
| 2026-05-30 | QTN-MNDesign itemized arrival → verification gap — clicking "arrived" on an item did not progress to arrival verification or inventory verification | `advanceFromEnRouteToVerificationIfAllDispatched` required `allFinished` (all items production_status === 'finished') in addition to `allDispatched`. In itemized progression, items can be dispatched/arrived before all are finished production, blocking the en_route → en_route_verification transition. | Removed `allFinished` check — only `allDispatched` is required for en_route → en_route_verification advancement. | Roo (Code) | Fixed |
