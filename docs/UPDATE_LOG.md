...
| 2026-05-28 00:54 | Roo (Code) | Fix: draggable chatbox — two bugs fixed: (1) click-outside handler now skips during drag via dragRef.current.isDragging check; (2) transform: translate() moved from outer container to chat panel only, so floating button always stays at bottom-6 right-6 | 🔴 Active |
