...
| `(pending)` | Codex | feat: Acknowledgement Receipts section on Collection page — new AcknowledgementReceiptsSection component with full table (receipt #, order #, client, payment type, amount, date, status, Download PDF button). Integrated refresh calls after every payment action. | 🔴 Deploying |
